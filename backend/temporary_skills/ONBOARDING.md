...
Agent instructions: when a teammate starts a session and asks to onboard (or runs this file),
**first ask which role they hold**, then give them the personalized section below. Do not dump
the whole file. Ask, then show only their role block plus the shared essentials.

Read `CLAUDE.md` (code spec) and `BUILD_PLAN.md` (full plan) for context before advising.
...
CLAUDE.md) with Backend and Frontend early.
...
You depend on: the `POST /turn` contract (CLAUDE.md). Agree it with Backend/AI Lead 1 before building.
...
- **Authoritative contracts** (turn endpoint, state, dice, trace) live in `CLAUDE.md`. Build against them so the pieces fit.
